...
08/2022 AGRT - modified hacked 4 beam configuration! To read properly from input file!
*/
    ...
    for (int t = 1; t < IMPACT_Heating::diffsteps; ++t)
    {
        for (int i = 1; i <= conf->Nx(); ++i)
        {
            for (int j = 1; j <= conf->Ny(); ++j)
            {
                ...
                tempint.set(i, j, IMPACT_Heating::i_mat.get(i, j) + sx * (IMPACT_Heating::i_mat.get(iplus, j) - 2 * IMPACT_Heating::i_mat.get(i, j) + IMPACT_Heating::i_mat.get(iminus, j)) + sy * (IMPACT_Heating::i_mat.get(i, jplus) - 2 * IMPACT_Heating::i_mat.get(i, j) + IMPACT_Heating::i_mat.get(i, jminus)));
            }
        }
        for (int i = 1; i <= conf->Nx(); ++i)
        {
                  ...
                  double eps, double ds, bool &flag, bool &firststep)
{
    ...
    if (r_temp.Get(1) < conf->xb(conf->Nx()) && r_temp.Get(2) < conf->yb(conf->Ny()) && r_temp.Get(1) > conf->xb(0) && r_temp.Get(2) > conf->yb(0))
    {
...
void tracerunner(IMPACT_Matrix &n_mat, IMPACT_Matrix &Bz_mat, IMPACT_StenOps *sten, IMPACT_Config *conf, IMPACT_MPI_Config *M)
{
    ...
    if (!ifnotdump && !(M->rank()))
        raystream.open(raydmpname.c_str());

    bool bool_u = 0, bool_s = 0, bool_g = 0;

    if (IMPACT_Heating::shape == 0)
    {
        bool_u = 1;
        bool_s = 0;
        bool_g = 0;
    }
    if (IMPACT_Heating::shape == 1)
    {
        bool_u = 0;
        bool_s = 1;
        bool_g = 0;
    }
    if (IMPACT_Heating::shape == 2)
    {
        bool_u = 0;
        bool_s = 0;
        bool_g = 1;
    ...
    for (r = local_ray_ind; r <= local_ray_total; ++r)
    {
        ...
        //std::cout << theta_live << std::endl;
        sigma = IMPACT_Heating::beam_width / sqrt(8.0 * log(2.0));// / sin(theta_live);
        ...
        if (!currentraydir)
        {
            ...
            livemult = livemult * (conf->xpos(upp) - conf->xpos(low)) * dy_orig *
                       fabs(rayvec.Get(4)) * ((bool_u) + (bool_s * (pow(sin((rayvec.Get(1) - x0) / IMPACT_Heating::beam_width * globalconsts::pi), 2))) + (bool_g * exp(-1.0 / 2.0 * pow((rayvec.Get(1) - xc) / sigma, 4.0))));
        ...
        else
        {
            ...
            livemult = livemult * (conf->ypos(upp) - conf->ypos(low)) * dx_orig * fabs(rayvec.Get(3)) * ((bool_u) + (bool_s * (pow(sin((rayvec.Get(2) - y0) / IMPACT_Heating::beam_width * globalconsts::pi), 2))) +
        ...
        if (airyflag)
        {
            ...
            if (IMPACT_Heating::direction)
            {
                if (temploc > conf->ypos(conf->Ny()))
                {
                    temploc = conf->ypos(conf->Ny());
                }
                tempeps = interp2(conf, eps_mat, conf->xpos(floor(conf->Nx() / 2.0)), temploc);
            }
            else
            {
                if (temploc > conf->xpos(conf->Nx()))
                {
                    temploc = conf->xpos(conf->Nx());
                }
        ...
        if (!discard)
            updatelocalintensity(tempint, localint);
        tempint.setall(0.0);
