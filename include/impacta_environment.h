...
namespace globalconsts
{
    ...
    std::string IMPACTA_numbers[nnumbers] = {"1", "2", "3", "4", "5", "6", "7", "8", "9", "0", ".", "LX", "LY", "PI", "X", "Y"};
    
    const bool if_use_hybrid_nonlinear = true;
}
namespace IMPACTA_ions
{
    ...
    double alpha_ion = (globalconsts::me_over_mp / atomic_mass_number / (globalconsts::oneover_atomic_Z));
...
std::string IMPACT_Get_CmdLine(int *argnum, char **argstr, std::string string)
{
    ...
    if (dircheck > 0)
    {
        answer = argstr[dircheck + 1];
        argstr[dircheck + 1] = null;
    }
    else
        answer = "*nofield*";
    return answer;
...
void IMPACT_dofunction(double *gridtemp, int N, int result)
{
    ...
    for (int i = 0; i < N; ++i)
        gridtemp[i] = IMPACT_function(gridtemp[i], result);
}
...
double IMPACT_isConst(char *number, double max, int N)
{
    ...
    double result = 0;
    if (!strcmp(number, "PI"))
        result = globalconsts::pi;
    if (!strcmp(number, "MAX"))
    {
        if (max == 0.0)
        {
            std::cout << "IMPACT: ERROR - in input deck, max requested but max - min = 0.0\n";
            exit(0);
        }
...
void chk(int check)
{
    std::cout << "EXIT OK - " << check << std::endl;
    exit(0);
}
...
namespace with_vec_err_checking
{
    ...
    inline void Vec_check_err(int *element, int *size)
    {
        ...
        catch (int err)
        {
            ...
            {
                std::cout << std::endl
                          << "Exiting - Tried to access non-existent vector element";
                std::cout << std::endl;
